...
from qr_code_login import QRCodeLogin
...
class JdSeckill(object):
    ...
    def __init__(self):
        ...
        self.user_session = UserSession(self.config)
        self.user_session.load_cookies_from_local()
        self.qrcode_login = QRCodeLogin(self.user_session)

        # 初始化信息
        self.sku_id = self.config.get('config', 'sku_id')
        self.sku_num = self.config.get('config', 'sku_num')
        self.seckill_init_info = dict()
        self.seckill_url = dict()
        self.seckill_order_data = dict()
        self.buy_time = datetime.strptime(self.config.get('config', 'buy_time'), "%Y-%m-%d %H:%M:%S.%f")
        self.timers = Timer()

        self.session = self.user_session.get_session()
        self.user_agent = self.user_session.user_agent
        self.nick_name = None
    ...
    def _seckill(self):
        ...
        while True:
            ...
            print(current_time)
            print(self.buy_time)
            print((current_time - self.buy_time).total_seconds())
